...
- **Price history & the 1000-row cap:** `price_history` holds one row per held item per day (02:00 UTC cron, `snapshot_price_history()`). **Never select those rows raw to build a per-user series** — PostgREST enforces `max-rows = 1000` server-side, and neither `.limit()` nor `.range()` can raise it. Read them through the aggregating RPCs instead: `portfolio_value_history(p_user_id)` (one row per date, feeds the value chart via `toPricePoints`) and `latest_prior_snapshots(p_user_id, p_window_days)` (`distinct on`, one row per item, feeds the day-over-day ticker in `lib/vaultDaily.ts`). Both are `security invoker`, so RLS scopes them to the caller while the service-role digest cron still works. *(History: the raw select was ordered oldest-first, so once an account passed 1000 item-days it silently lost its most RECENT weeks — a 3,990-row collection's chart ended 25 days early, the 7D window came back "not enough data", and the longer windows drew a straight line across the gap. The same truncation fed the "today" delta a 25-day-old baseline.)*
- **Day-over-day change (`lib/vaultDaily.ts`) is snapshot-first.** `computeDailyChanges` diffs the `latest_prior_snapshots` value against the item's current `market_price`; the provider's `priceChange24hr` (off `card_prices.raw`) is only a **fallback**, used when an item has no snapshot yet — a card added since the last 02:00 run — and only when `card_prices.updated_at` is within 24h. **Never treat `priceChange24hr` as "today's move" without checking that timestamp:** it measures the 24h *before* the row was written, and nothing refreshes `card_prices` on a schedule. *(History: it used to win outright with no age check. A 6-day-old −12.36% on a $7.73 × 2 holding kept regenerating the same −$2.18 — stale pct × unchanged `market_price` is a constant — so the daily digest pushed "down $2.18, led by Ampharos" six days running, while `price_history` showed one real −$1.84 move and flat after. Snapshot-first is also self-correcting: once the price settles, the next snapshot equals it and the item drops out on its own. It additionally guarantees the digest and the value chart agree, since both now read `price_history`.)* Corollary: `market_price` only moves on `/api/market-refresh` or the lazy `/api/card-price` path — **there is no market-refresh cron** — so on days with no refresh the whole vault is genuinely flat and the digest correctly stays silent.
