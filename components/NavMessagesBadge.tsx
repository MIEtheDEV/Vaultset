...
export function NavMessagesBadge() {
  ...
  useEffect(() => {
    ...
    async function fetchUnread(userId: string) {
      ...
      if (!convIds.length) { setCount(0); return; }
    ...
    function setupForUser(userId: string) {
      ...
      channel = supabase
        .channel(`nav-messages-badge-${userId}`)
        .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, () => fetchUnread(userId))
        .on("postgres_changes", { event: "UPDATE", schema: "public", table: "messages" }, () => fetchUnread(userId))
        .subscribe();
    }
