...
export function PortfolioAnalyticsClient({
  ...
  const filteredHistory = windowSlice(portfolioHistory, range);
  const lastValue = filteredHistory.at(-1)?.value ?? 0;
  const rangeDelta = windowChange(filteredHistory);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-surface p-6">
        <div className="flex items-start justify-between flex-wrap gap-3 mb-5">
          <div>
            ...
            {rangeDelta ? (
              <div className="flex items-baseline gap-2 mt-1">
                ...
                {/* Same wording and colour tokens as the dashboard's range delta. */}
                <span
          ...
          <TrendRangePicker value={range} onChange={setRange} />
        </div>
        ...
        {!rangeDelta ? (
          ...
          <>
            {/* The chart's only second mark, so it gets a legend. */}
            {showCostBasisLine && (
            ...
            <ValueTrendChart
              ...
              referenceValue={showCostBasisLine ? totalCostBasis : null}
            />
          </>
        )}
      </div>
