...
export default async function FollowingPage({
  ...
  const profileMap = new Map((followingProfiles ?? []).map((p: any) => [p.id, p]));
  const orderedProfiles = followingIds.map((id) => profileMap.get(id)).filter(Boolean);
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: "Following", item: `https://www.vaultset.app/profile/${username}/following` },
    ],
  };
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-foreground-muted">
          @{username} is following {followingIds.length} {followingIds.length === 1 ? "collector" : "collectors"}
        </p>
      </div>

      {orderedProfiles.length === 0 ? (
        <div className="rounded-2xl border border-border bg-surface py-16 text-center">
          <p className="text-sm text-foreground-muted">Not following anyone yet.</p>
        </div>
      ) : (
