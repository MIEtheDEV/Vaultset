...
const EX_SUBTYPES = ["ex", "mega", "gx", "v", "vmax", "vstar"];

/**
 * Whether a Pokémon card is an "ex"-class card (ex / EX / GX / V / VMAX / VSTAR /
 * Mega). Uses pokemontcg.io `subtypes` when present, and otherwise infers from the
 * card name's suffix — so the flag stays correct for scan matches that arrive
 * without subtypes (fromIndex / `tcg:` / enrichment timeouts), where it would
 * previously always come back false.
 */
export function deriveIsEx(name: string, subtypes?: string[]): boolean {
  ...
  // "Charizard VMAX", "Arceus VSTAR".
  return /\s(?:ex|gx|v|vmax|vstar)$/i.test(name.trim());
}
